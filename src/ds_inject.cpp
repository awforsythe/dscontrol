...
bool ds_inject::warp_to_bonfire(gp_process& process, const ds_memmap& memmap, uint32_t bonfire_id)
{
	if (!process.poke<uint32_t>(memmap.chr_class_warp.last_bonfire, bonfire_id))
	{
		return false;
	}

	memcpy(s_bonfire_warp_instructions + BONFIRE_WARP_CHR_CLASS_OFFSET, &memmap.bases.chr_class, sizeof(void*));
	memcpy(s_bonfire_warp_instructions + BONFIRE_WARP_BONFIRE_OFFSET, &memmap.bases.bonfire_warp, sizeof(void*));
